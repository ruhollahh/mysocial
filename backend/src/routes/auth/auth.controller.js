...
async function authController(req, res) {
  const { firstName, lastName, handle, email } = req.user;
  return res.status(200).json({ user: { firstName, lastName, handle, email } });
}
...
async function signupController(req, res) {
  ...
  try {
    ...
    const { firstName, lastName, handle, email } = user;
    return res
      .status(201)
      .json({ user: { firstName, lastName, handle, email } });
  } catch (error) {
